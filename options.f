0 value opt-len
create opt-name 64 allot

0 value type-len
create type-name 64 allot

: clear-opt opt-name 64 bl fill ;

: next-num [char] ) word count type space ;

: const-name clear-opt [char] , word count to opt-len opt-name opt-len move ;
...
: make-curlopts
        begin refill while
                const-name 
                bl skip [char] , word count to type-len type-name type-len move
                bl skip next-num tab 
                type-name type-len type tab type-len 24 < if tab then  
		." CURLOPT" tab opt-name opt-len type cr
        repeat
        bye ;
make-curlopts
